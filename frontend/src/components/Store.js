        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredProducts.map((product) => (
            <div key={product.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden">
              <div className="flex h-56 min-h-[224px]">
                {/* Product Image - Left side */}
                <div className="relative w-40 h-56 flex-shrink-0 overflow-hidden">
                  <img
                    src={product.image}
                    alt={product.name}
                ...
                {/* Product Info - Right side - EXTRA TALL FOR BUTTON */}
                <div className="flex-1 p-4 flex flex-col justify-between min-w-0">
                  ...
                  {/* Add to cart section - WELL POSITIONED AT BOTTOM */}
                  <div className="flex items-center justify-center w-full pt-4 border-t border-gray-100">
                    {(cart && cart[product.id] && cart[product.id] > 0) ? (
                      ...
                      <button
                        onClick={() => addToCart(product.id)}
                        className="bg-neon-blue hover:bg-blue-600 text-white px-6 py-3 rounded-lg font-medium transition-colors text-sm flex items-center justify-center w-full max-w-[200px]"
                      >
